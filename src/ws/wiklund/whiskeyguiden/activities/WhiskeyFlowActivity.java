...
public class WhiskeyFlowActivity extends BaseActivity implements Notifyable {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		if (getResources().getConfiguration().orientation != Configuration.ORIENTATION_LANDSCAPE) {
			startActivity(new Intent(getApplicationContext(), WhiskeyListActivity.class));
		} else {
			...
			flow.setOnItemClickListener(new OnItemClickListener() {
				...
				public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
					new GetBeverageFromCursorTask(WhiskeyFlowActivity.this, WhiskeyActivity.class).execute(adapter.getItem(position));
				}				
			});
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		...
		case R.id.menuStats:
			startActivity(new Intent(WhiskeyFlowActivity.this.getApplicationContext(), StatsActivity.class));
			break;
		case R.id.menuExport:
			final AlertDialog alertDialog = new AlertDialog.Builder(WhiskeyFlowActivity.this).create();
			...
			alertDialog.setButton(DialogInterface.BUTTON_POSITIVE, getString(android.R.string.yes), new DialogInterface.OnClickListener() {
				public void onClick(DialogInterface dialog, int which) {
			       new ExportDatabaseCSVTask(WhiskeyFlowActivity.this, helper, exportFile, adapter.getCount()).execute();
				} 
			});
			...
			break;
		case R.id.menuAbout:
			startActivity(new Intent(WhiskeyFlowActivity.this.getApplicationContext(), AboutActivity.class));
			break;
		}
	...
	private void handleLongClick(final int position) {
		...
		alertDialog.setSingleChoiceItems( selectableAdapter, 0, new OnClickListener() { 
            ...
            public void onClick(DialogInterface dialog, int which) { 
                dialog.dismiss();
                ((Selectable) selectableAdapter.getItem(which)).select(WhiskeyFlowActivity.this, helper, b);
            }
		}); 
	...
	public void notifyDataSetChanged() {
		...
		if (bottles > 0) {
			TextView view = (TextView) WhiskeyFlowActivity.this.findViewById(R.id.title);
